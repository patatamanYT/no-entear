...
<style>
  * { box-sizing: border-box; }
  ...
  .heading { font-family:'Poppins','Segoe UI',Arial,sans-serif; font-weight:900; letter-spacing:-.4px; color:#1F2A44; -webkit-font-smoothing:antialiased; -moz-osx-font-smoothing:grayscale; text-rendering:optimizeLegibility; }
...
<body>
  <div class="container">
    ...
    <div style="padding:6px 30px 4px 30px;">
      <div class="heading" style="font-size:36px; line-height:1.15; margin-bottom:8px;">${renderTitulo(titulo, tituloResaltado)}</div>
...
${strippedHtml}`;
  ...
  function fitScaledIframe(iframe, wrap, tries, lastHeight) {
    ...
    if (h > 0 && h === lastHeight) {
      ...
      // zoom (no transform:scale) para achicar el preview: transform solo
      // reescala los pixeles ya renderizados (como una imagen encogida),
      // asi que el texto se ve mas delgado/suave que el tamaño real -- zoom
      // hace que el navegador vuelva a rasterizar el texto a ese tamaño
      // efectivo, con el mismo grosor relativo que el PNG exportado.
