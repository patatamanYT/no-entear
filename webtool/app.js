  ...
  <div class="container">
    ...
    <div style="padding:20px 30px 8px 30px; background:#FFFFFF; display:flex; justify-content:space-between; align-items:center;">
      <div style="display:flex; align-items:center; gap:12px;">
        ...
        <img src="[[ASSET_WORDMARK]]" alt="Lo Mejoramos Juntos" style="height:46px; width:auto; display:block;">
...
${renderNotaCierre(notaCierre)}
  ...
  // [[ASSET_LOGO]] / [[ASSET_WORDMARK]] en vez de base64 -- se resuelven
  // aqui mismo (nunca se manda un base64 de 100KB+ a Claude al preparar
  // la traduccion). Se usan corchetes dobles y NO guiones bajos dobles
  ...
  function resolveAssets(html, lang) {
    ...
    const resolved = html.split('[[ASSET_LOGO]]').join(LOGO_URI).split('[[ASSET_WORDMARK]]').join(wordmark);
...
- Los marcadores literales [[ASSET_LOGO]] y [[ASSET_WORDMARK]] deben quedar EXACTAMENTE igual, en su misma posición dentro del atributo src. No los traduzcas, no los muevas, no los elimines, no les quites ni agregues corchetes.
...
${strippedHtml}`;
  ...
  function strippedHtmlForPrompt() {
    // el HTML ya sale con [[ASSET_LOGO]]/[[ASSET_WORDMARK]] en vez de
  ...
  function setupTranslateLang(lang) {
    ...
    btnRender.addEventListener('click', () => {
      ...
      if (!pasted.includes('[[ASSET_LOGO]]') || !pasted.includes('[[ASSET_WORDMARK]]')) {
        showMsg(statusEl, 'error', 'El HTML pegado no conserva los marcadores [[ASSET_LOGO]]/[[ASSET_WORDMARK]] — Claude pudo haberlos alterado al traducir. Pídele que responda de nuevo dejando esos marcadores exactamente igual, entre corchetes dobles.');
        return;
      }
